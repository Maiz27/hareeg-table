...
/// Generated first-party Warm Sudanese Lounge deck. WebP-backed faces, with
/// the broken J♦ slot routed back to the code-rendered painter.
const HareegCardTheme sandlineLoungeCardTheme = HareegCardTheme(
  ...
  assetManifest: CardThemeAssetManifest(
    ...
    extension: 'webp',
    // The bundled `jack_diamonds.webp` is a duplicate of `jack_hearts.webp`
    // (a red Jack with a heart corner pip). Until corrected court art ships
    // we route J♦ through the code-rendered painter so the painted rank +
    // pip match the card's identity. The asset-consistency test enforces
    // this skip with the `sandline_lounge:jack:diamonds` known-bad entry.
    skipFaces: [
      CardIdentity(rank: CardRank.jack, suit: CardSuit.diamonds),
    ],
  ),
);
